...
USE GroceryStore;
-- Create the parent table for Addresses first
...
-- Create the parent table for Categories
CREATE TABLE Categories (
    category_id INT AUTO_INCREMENT PRIMARY KEY,
    category_name VARCHAR(50) NOT NULL
);

-- Create the child table for Customers (references Addresses)
...
-- Create the child table for Products (references Categories)
CREATE TABLE Products (
    product_id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    price DECIMAL(10,2) NOT NULL, -- This is now the unit price
...
-- Create the child table for Orders (references Customers)
CREATE TABLE Orders (
...
-- Create the child table for Payments (references Orders)
...
-- Create the child table for Cart (references Customers and Products)
-- 'price' is now the line item total and will be calculated by the trigger
CREATE TABLE Cart (
...
-- Step 4: Create the trigger to automatically calculate the total price for each cart item
DELIMITER $$
...
-- Step 5: Insert the data into the tables
...
-- Insert Sample Orders (total_amount and total_quantity are left as NULL to be updated later)
INSERT INTO Orders (customer_id, total_amount, total_quantity)
VALUES
(1, NULL, NULL),
(2, NULL, NULL);


-- Now, when you insert into the Cart, you do not need to provide the 'price'
INSERT INTO Cart (customer_id, product_id, quantity)
VALUES
(1, 1, 2), -- The price will be calculated automatically by the trigger
(1, 4, 1),
(2, 2, 3),
(2, 7, 2),
(2, 8, 1);

-- Update the total_amount and total_quantity in the Orders table
...
-- Insert Payments using a subquery to get the total_amount from Orders
-- This is a much better approach than manually entering the amount
